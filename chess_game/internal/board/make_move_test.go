	...
	moveTests := []struct {
		...
		// ─── Pawn moves ─────────────────────────────────────────────────────
		{
			...
			afterMove: func(t *testing.T, game *pieces.GameState) {
				row, col := utils.Chess_notation_to_indices("e4")
				if !game.Board[row][col].Occupied {
					t.Error("expected e4 to be occupied after pawn move")
				}
				row, col = utils.Chess_notation_to_indices("e2")
				if game.Board[row][col].Occupied {
		...
		// ─── Piece moves ────────────────────────────────────────────────────
		{
			...
			expectedErr:   false,
			afterMove: func(t *testing.T, game *pieces.GameState) {
				row, col := utils.Chess_notation_to_indices("a4")
				if !game.Board[row][col].Occupied {
					t.Error("expected a4 to be occupied after rook move")
				}
				row, col = utils.Chess_notation_to_indices("a1")
				if game.Board[row][col].Occupied {
		...
		// ─── Capture moves ──────────────────────────────────────────────────
		{
			...
			expectedErr:   false,
			afterMove: func(t *testing.T, game *pieces.GameState) {
				row, col := utils.Chess_notation_to_indices("d5")
				if !game.Board[row][col].Occupied {
